...
local function shown(state)
    return state.shown[#state.shown]
end

it("moves a sentence prompt on after a different word, noting it",
        function()
    ...
    T.eq(shown(state), "you [like] to (1/1)")
    swipe(recorder, "like", { { word = "Like" } })
    T.eq(last(state).matched, true)
    T.eq(last(state).word_index, 2)
end)

it("moves on after any typed word in words mode", function()
    local recorder, state = newRecorder({ "water", "hello" })
    recorder:start()
    swipe(recorder, "wafer", { { word = "wafer" } })
    T.eq(last(state).matched, false)
    T.eq(last(state).word_index, nil)
    T.eq(shown(state), "Swipe: hello (2/2)")
end)
