...
local function held(state)
    return state.shown[#state.shown]
end

it("holds a sentence prompt when a different word was typed", function()
    local recorder, state = newRecorder({ "you like to" }, "sentences")
    recorder:start()
    swipe(recorder, "yu", { { word = "like" } })
    T.eq(last(state).target, "you")
    T.eq(last(state).matched, false)
    T.eq(last(state).word_index, 1)
    T.truthy(held(state):find('Typed "like", not "you"', 1, true),
        "names both words: " .. held(state))
    T.truthy(held(state):find("[you] like to (1/1)", 1, true),
        "still prompts you: " .. held(state))
end)

it("moves on when the prompted word is picked, not another", function()
    local recorder, state = newRecorder({ "you like to" }, "sentences")
    recorder:start()
    swipe(recorder, "yu", { { word = "like" } })
    recorder:picked(3, "yo")
    T.truthy(held(state):find('Typed "like"', 1, true), "still held")
    recorder:picked(2, "You")
    T.eq(held(state), "you [like] to (1/1)")
end)

it("asks again after the wrong word is deleted", function()
    local recorder, state = newRecorder({ "you like to" }, "sentences")
    recorder:start()
    swipe(recorder, "yu", { { word = "like" } })
    recorder:deleted()
    T.eq(held(state), "[you] like to (1/1)")
    swipe(recorder, "you", { { word = "you" } })
    T.eq(last(state).after_uncorrected, nil)
    T.eq(held(state), "you [like] to (1/1)")
end)

it("flags a swipe made while a wrong word was left in place", function()
    local recorder, state = newRecorder({ "you like to" }, "sentences")
    recorder:start()
    swipe(recorder, "yu", { { word = "like" } })
    local wrong = last(state).id
    swipe(recorder, "to", { { word = "to" } })
    T.eq(last(state).target, "you")
    T.eq(last(state).after_uncorrected, wrong)
end)

it("still moves on after any typed word in words mode", function()
    local recorder, state = newRecorder({ "water", "hello" })
    recorder:start()
    swipe(recorder, "wafer", { { word = "wafer" } })
    T.eq(last(state).matched, false)
    T.eq(last(state).word_index, nil)
    T.eq(held(state), "Swipe: hello (2/2)")
end)
