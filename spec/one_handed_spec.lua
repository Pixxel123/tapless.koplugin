...
it("lays out the keys and puts the panel in the wider strip", function()
    local right = OneHanded.layout(RIGHT, PORTRAIT, 4, 1256)
    T.eq(right.keys_w, 803, "keys width")
    T.eq(right.before, 453, "before")
    T.eq(right.after, 0, "after")
    T.eq(right.panel_side, "left", "panel side")
    T.eq(right.panel_w, 453, "panel width")
    local left = OneHanded.layout(LEFT, PORTRAIT, 4, 1256)
    T.eq(left.before, 0, "left before")
    T.eq(left.after, 453, "left after")
    T.eq(left.panel_side, "right", "left panel side")
end)

it("sizes square buttons at 19 mm unless there is no room", function()
    T.eq(OneHanded.buttonSize(PORTRAIT, 453, 700, 2), 224, "room")
    T.eq(OneHanded.buttonSize(PORTRAIT, 150, 700, 2), 150, "narrow strip")
    T.eq(OneHanded.buttonSize(PORTRAIT, 453, 500, 2), 165, "short keyboard")
end)
